...
checksmbinstall() {
  ...
  if [[ ! -f "$smb_conf" ]]; then
    printf "\n⚠️ Achtung: Die Konfigurationsdatei für 'Samba-Server' wurde nicht gefunden!\n"
    printf "\n📍 Pfad: %s\n\n" "$smb_conf"
    printf "\n💾 'Samba' ist eventuell nicht installiert. - Bitte erst installieren.\n\n\n"
    return 1
  fi
  ...
  if [[ ! -s "$smb_conf" ]]; then
    ...
    printf "\n💾 'Samba' ist eventuell nicht richtig installiert. - Bitte noch einmal installieren.\n\n\n"
    return 1
  fi
}
